...
function PlayerRow({ player, average, participants, displayRank, compact = true }) {
  ...
  const rank = Number.isFinite(player.rank) ? player.rank : displayRank
  const podiumIcon = getPodiumIcon(rank)
    ...
    <div className={`player-row ${above ? 'above' : 'below'} ${compact ? 'compact' : ''}`}>
      <div className="player-main">
        ...
        <div className="player-score-wrap">
          <span className={`rank-badge ${podiumIcon ? 'has-podium' : ''}`} title={`Pozycja w wojnie: #${rank}`}>
            {podiumIcon ? <span className="podium-icon" aria-hidden="true">{podiumIcon}</span> : null}
            ...
            {sortedPlayers.map((player, index) => {
              ...
              return (
                <div key={`${war.id}-${player.player}`}>
                  ...
                  <PlayerRow player={player} average={average} participants={war.participant_count} displayRank={index + 1} />
                </div>
              )
            })}
          </div>
  ...
  return (
    <div className="app-shell">
      <header className="app-header">
        <div>
          <h1>EP War Bot</h1>
          <p>v3.4.66 • kompaktowe szczegóły wojny • tryb przy wyniku • średnia • podium top 3</p>
        </div>
      </header>
